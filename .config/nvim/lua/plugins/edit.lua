return {
  ...
  -- [nvim-surround - 괄호/따옴표/태그 감싸기 끝판왕]
  -- ── 핵심 키맵 ────────────────────────────────────────────────────────────
  --   노멀: ys{모션}{구분자}  → 텍스트를 구분자로 감싸기
  --   노멀: ds{구분자}        → 둘러싸는 구분자 삭제
  --   노멀: cs{기존}{새것}    → 둘러싸는 구분자 교체
  --   비주얼: S{구분자}       → 선택 영역을 구분자로 감싸기
  -- ── 실전 예시 ────────────────────────────────────────────────────────────
  --   ds"          : "hello"          → hello
  --   cs"'         : "hello"          → 'hello'
  --   ysiw)        : hello            → (hello)
  --   ysiw<div>    : hello            → <div>hello</div>
  --   cst<span>    : <div>hi</div>    → <span>hi</span>
  -- ── Textobjects 결합 ─────────────────────────────────────────────────────
  --   ysaf)        : 함수 전체(af)를 괄호로 감싸기
  --   ysif"        : 함수 내부(if)를 따옴표로 감싸기
