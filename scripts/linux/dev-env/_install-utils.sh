...
# ─────────────────────────────────────────────────────────────────
# 🎨 Gradle Spotless 포매터 전역 설정 공용 함수
# ─────────────────────────────────────────────────────────────────
configure_gradle_spotless() {
    echo "---------------------------------------------------------------------------"
    print_step "🎨 9. Gradle Spotless (Java 포매터) 전역 설정"
    echo ""
    echo "   모든 Gradle Java 프로젝트에서 s2 표준 코드 스타일(Eclipse JDT 기반)을"
    echo "   Spotless 플러그인으로 빌드 시 자동 검증합니다."
    echo ""
    echo "   대상 파일: ~/.gradle/init.d/spotless.gradle"
    echo "   포맷 정의: ${DEVTOOLS2:-.}/.config/formatter/java/s2-code-style.xml"
    echo ""
    print_info "💡 사용 명령어:"
    echo "      ./gradlew spotlessCheck    — 포맷 위반 검사 (빌드 시 자동 실행)"
    echo "      ./gradlew spotlessApply    — 자동 포맷 적용"
    echo ""

    local GRADLE_INIT_DIR="$HOME/.gradle/init.d"
    local SPOTLESS_FILE="$GRADLE_INIT_DIR/spotless.gradle"
    local CODE_STYLE_PATH="${DEVTOOLS2:-/var/opt/_devtools2}/.config/formatter/java/s2-code-style.xml"

    if prompt_confirm "👉 Gradle Spotless 전역 포매터 설정을 추가할까요?" "N"; then
        mkdir -p "$GRADLE_INIT_DIR"

        # s2-code-style.xml 존재 확인
        if [ ! -f "$CODE_STYLE_PATH" ]; then
            print_warn "s2-code-style.xml 파일을 찾을 수 없습니다: $CODE_STYLE_PATH"
            print_warn "파일이 존재하는지 확인 후 다시 실행해 주세요."
            return 0
        fi
        ...
        if [ -f "$SPOTLESS_FILE" ]; then
            echo ""
            print_warn "파일이 이미 존재합니다: $SPOTLESS_FILE"
            if ! prompt_confirm "   기존 파일을 새 설정으로 교체할까요?" "N"; then
                do_write=false
                print_info "기존 파일을 유지합니다."
            fi
        ...
        if [ "$do_write" = "true" ]; then
            # Groovy heredoc 안에서 Gradle 변수(${project.xxx})가 쉘 변수 치환되지 않도록
            # 'SPOT_EOF' 형식(따옴표 감싸기)을 사용하되, CODE_STYLE_PATH만 선주입합니다.
            cat > "$SPOTLESS_FILE" << SPOT_EOF
// ─────────────────────────────────────────────────────────────────
// Gradle Spotless 전역 Init 스크립트 (자동 생성 by devtools2)
// 적용 대상: Java 소스를 포함하는 모든 Gradle 프로젝트
// 포맷 기준: s2-code-style.xml (Eclipse JDT Code Formatter)
// ─────────────────────────────────────────────────────────────────
initscript {
    repositories {
        gradlePluginPortal()
...
allprojects {
    afterEvaluate { project ->
        // java 또는 java-library 플러그인이 적용된 프로젝트에만 Spotless 적용
        if (project.plugins.hasPlugin('java') || project.plugins.hasPlugin('java-library')) {
            project.apply plugin: com.diffplug.gradle.spotless.SpotlessPlugin

            project.spotless {
                java {
                    target 'src/*/java/**/*.java'
                    eclipse().configFile('${CODE_STYLE_PATH}')
                    trimTrailingWhitespace()
                    endWithNewline()
                }
            }

            // compileJava 에 spotlessCheck 연결 — 빌드 시 자동 검증
...
SPOT_EOF
            print_done "Gradle Spotless 전역 포매터 설정 완료"
            echo "      파일: $SPOTLESS_FILE"
            echo "      스타일: s2-code-style.xml (Eclipse JDT)"
        fi
    else
        if [ -f "$SPOTLESS_FILE" ]; then
            print_info "기존 Gradle Spotless 설정을 유지합니다 (변경 없음): $SPOTLESS_FILE"
        else
            print_skip "Gradle Spotless 전역 포매터 설정을 건너뜁니다."
            echo "      나중에 추가하려면 $SPOTLESS_FILE 파일을 직접 생성하세요."
        fi
    fi
    echo ""
}
