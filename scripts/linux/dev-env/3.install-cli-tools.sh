...
_load_install_utils() {
    ...
    # 온라인 전용, 캐시 우회 헤더 포함 (이유는 _load_colors 주석 참고).
    # TOOL_VERSIONS_TOML 등은 인라인 대체가 불가능해서 실패하면 바로 하드 실패한다.
    local _curl_err _curl_ec=0
    _curl_err=$(curl -sSfL --max-time 5 -H 'Cache-Control: no-cache, no-store, must-revalidate' -H 'Pragma: no-cache' "https://raw.githubusercontent.com/devers2/_devtools2/main/scripts/linux/dev-env/_install-utils.sh" -o /tmp/_install_utils_remote.sh 2>&1) || _curl_ec=$?
    if [ "$_curl_ec" -eq 0 ]; then
        # shellcheck disable=SC1091
        source /tmp/_install_utils_remote.sh 2>/dev/null && _INSTALL_UTILS_LOADED=true && return 0
        print_error "_install-utils.sh를 다운로드했지만 source 실행 중 오류가 발생했습니다: /tmp/_install_utils_remote.sh"
        exit 1
    fi
