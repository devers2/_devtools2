...
if [ "$VERSION_MODE" = "individual" ]; then
    ...
    echo ""
    prompt_input "   선택 [1/${_C_GREEN}2${_C_RESET}]: "; read -r _nv_sel
    case "${_nv_sel:-2}" in
...
if [ "$NODEJS_INSTALLED" = true ]; then
    case "$DUPLICATE_MODE" in
        ...
        individual)
            ...
            prompt_input "   삭제 후 재설치하시겠습니까? [y/${_C_GREEN}N${_C_RESET}]: "; read -r _nd_dup
            case "${_nd_dup:-N}" in
...
if [ -f "package.json" ]; then
    ...
    if [ "$_has_npm_pkgs" = true ]; then
        ...
        _npm_choice="n"
        if [ -t 0 ]; then
            prompt_input "   글로벌 npm 패키지를 다시 복구(npm install)하시겠습니까? [y/${_C_GREEN}N${_C_RESET}]: "
            read -r _npm_choice
        fi
...
if [ -d "$DEVTOOLS2/modules/neovim/nvim" ]; then
    # 사용자에게 선택 입력 요청
    prompt_input "   ⚠️  neovim 디렉토리가 이미 존재합니다. 삭제하고 새로 설치하시겠습니까? [y/${_C_GREEN}N${_C_RESET}]: "; read -r choice
