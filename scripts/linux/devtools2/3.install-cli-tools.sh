...
echo "---------------------------------------------------------------------------"
echo "⌨️  9. keyd 설치 (CapsLock 키 매핑 수정)"
echo ""
echo "   ※ keyd 를 설치하면 아래와 같이 CapsLock 키를 재매핑합니다."
echo "      - 리눅스에서 CapsLock 상태일 때 Shift + 영문키를 눌러도 소문자가 되지 않는 문제가 있습니다."
echo "      - Neovim 에서 CapsLock 으로 대문자 상태를 만들면 소문자 단축키가 동작하지 않습니다."
echo "      - Neovim 의 ESC / Ctrl 키 사용 편의를 위해 아래와 같이 매핑합니다."
echo ""
echo "         CapsLock 단독 탭  →  ESC"
echo "         CapsLock + 다른 키 →  Ctrl (조합키)"
echo ""

if [ "$IS_WSL2" = true ]; then
    echo "   ⚠️  [WSL2 환경 감지] keyd 는 커널 수준에서 키보드를 제어하므로"
    echo "      WSL2 에서는 동작하지 않습니다. 건너뜁니다."
    echo ""
else
    echo -n "   keyd 를 설치할까요? [Y/n]: "
    read -r keyd_answer
    keyd_answer_lower=$(echo "${keyd_answer:-y}" | tr '[:upper:]' '[:lower:]')

    if [ "$keyd_answer_lower" = "y" ]; then
        ...
        if command -v keyd > /dev/null 2>&1; then
            echo ""
            echo "   ⚠️  keyd 가 이미 설치되어 있습니다."
            echo -n "   설정 파일만 재작성할까요? (재설치 없이 설정만 적용) [Y/n]: "
            read -r keyd_reconf
            keyd_reconf_lower=$(echo "${keyd_reconf:-y}" | tr '[:upper:]' '[:lower:]')
            if [ "$keyd_reconf_lower" != "y" ]; then
                echo "   ⏭️  keyd 설정을 유지합니다."
                echo ""
                # 서비스만 재시작
                sudo systemctl restart keyd 2>/dev/null && echo "   ✅ keyd 서비스 재시작 완료"
                echo ""
            else
                keyd_do_conf=true
            fi
            ...
            if command -v rpm-ostree > /dev/null 2>&1; then
                echo "   📦 rpm-ostree 환경 감지: keyd COPR 저장소 추가 및 설치..."
                sudo dnf5 copr enable alternateved/keyd -y
                rpm-ostree install keyd
                echo ""
                echo "   ⚠️  rpm-ostree 특성상 설치 반영을 위해 재부팅이 필요합니다."
                echo "      재부팅 후 keyd 서비스가 자동으로 활성화됩니다."
                echo ""
                echo -n "   지금 바로 재부팅할까요? [Y/n]: "
                read -r reboot_answer
                reboot_answer_lower=$(echo "${reboot_answer:-y}" | tr '[:upper:]' '[:lower:]')
                if [ "$reboot_answer_lower" = "y" ]; then
                    echo "   재부팅합니다..."
                    sudo systemctl reboot
                else
                    echo "   ⏭️  나중에 직접 재부팅 후 keyd 설정을 적용해 주세요."
                    echo ""
                fi
            elif command -v apt-get > /dev/null 2>&1; then
                echo "   📦 apt 환경 감지: keyd 설치 중..."
                sudo apt-get install -y keyd -qq
            elif command -v dnf > /dev/null 2>&1; then
                echo "   📦 dnf 환경 감지: keyd 설치 중..."
                sudo dnf install -y keyd
            else
                echo "   ❌ 지원하는 패키지 매니저를 찾을 수 없습니다. 수동으로 keyd 를 설치해 주세요."
                echo ""
            fi
            keyd_do_conf=true
        fi
...
[main]
# 짧게 누르면 esc, 다른 키와 조합하면 control
capslock = overload(control, esc)

# 오른쪽 컨트롤 키를 명시적으로 지정하여 keyd 에서 정확하게 인식할 수 있도록 함
rightcontrol = rightcontrol
EOF
                echo "   ✅ keyd 설정 파일 작성 완료: $KEYD_CONF_FILE"
            fi

            # 서비스 활성화 및 재시작
            if command -v keyd > /dev/null 2>&1; then
                sudo systemctl enable --now keyd 2>/dev/null
                sudo systemctl restart keyd 2>/dev/null
                echo "   ✅ keyd 서비스 활성화 및 시작 완료"
                echo ""
                echo "   📌 적용된 키 매핑:"
                echo "      CapsLock 단독 탭  →  ESC"
                echo "      CapsLock + 다른 키 →  Ctrl"
            fi
        fi
    else
        echo "   ⏭️  건너뜀: 나중에 설치하려면 아래 명령어를 참고하세요."
        echo "      sudo apt-get install keyd  # Ubuntu/Debian"
        echo "      sudo dnf install keyd      # Fedora"
    fi
fi
echo ""
