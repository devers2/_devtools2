...
if [ "$_HAS_INSTALLED" = true ]; then
    ...
    echo ""
    prompt_input "   선택 [1/${_C_GREEN}2${_C_RESET}/3]: "; read -r _dup_choice
    echo ""
...
echo ""
prompt_input "   선택 [1/${_C_GREEN}2${_C_RESET}/3]: "; read -r _ver_choice
echo ""
...
_resolve_action() {
    ...
    case "$DUPLICATE_MODE" in
        ...
        individual)
            ...
            prompt_input "   삭제 후 재설치하시겠습니까? [y/${_C_GREEN}N${_C_RESET}]: "; read -r _dup_sel
            case "${_dup_sel:-N}" in
                y|Y) echo "reinstall" ;;
                *)   echo "skip" ;;
            esac
...
if [ "$VERSION_MODE" = "individual" ]; then
    ...
    echo ""
    prompt_input "   선택 [1/${_C_GREEN}2${_C_RESET}]: "; read -r _fzf_vs
    case "${_fzf_vs:-2}" in
...
if [ "$VERSION_MODE" = "individual" ]; then
    ...
    echo ""
    prompt_input "   선택 [1/${_C_GREEN}2${_C_RESET}]: "; read -r _lg_vs
    case "${_lg_vs:-2}" in
...
if [ "$VERSION_MODE" = "individual" ]; then
    ...
    echo ""
    prompt_input "   선택 [1/${_C_GREEN}2${_C_RESET}]: "; read -r _rg_vs
    case "${_rg_vs:-2}" in
...
if [ "$VERSION_MODE" = "individual" ]; then
    ...
    echo ""
    prompt_input "   선택 [1/${_C_GREEN}2${_C_RESET}]: "; read -r _fd_vs
    case "${_fd_vs:-2}" in
...
if [ "$VERSION_MODE" = "individual" ]; then
    ...
    echo ""
    prompt_input "   선택 [1/${_C_GREEN}2${_C_RESET}]: "; read -r _sg_vs
    case "${_sg_vs:-2}" in
...
if [ "$VERSION_MODE" = "individual" ] && [ "$IS_ARM64" = true ]; then
    ...
    echo ""
    prompt_input "   선택 [1/${_C_GREEN}2${_C_RESET}]: "; read -r _bw_vs
    case "${_bw_vs:-2}" in
...
if [ "$IS_WSL2" = true ]; then
    ...
    if [ "$VERSION_MODE" = "individual" ]; then
        ...
        echo ""
        prompt_input "   선택 [1/${_C_GREEN}2${_C_RESET}]: "; read -r _wy_vs
        case "${_wy_vs:-2}" in
...
echo ""
prompt_input "❓ Gradle bootRun DAP Attach 모드 전역 설정을 추가할까요? [${_C_GREEN}Y${_C_RESET}/n]: "; read -r dap_answer
...
if [ "$dap_answer_lower" = "y" ]; then
    ...
    if [ -f "$GRADLE_DEBUG_FILE" ]; then
        ...
        prompt_input "   기존 파일을 새 설정으로 교체할까요? [y/${_C_GREEN}N${_C_RESET}]: "; read -r overwrite_answer
