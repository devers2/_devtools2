#!/bin/bash
# ==============================================================================
# 사용법:
#   ./create-symbolic-link.sh <원본_경로> <생성할_링크_경로>
#
# 설명:
#   지정한 원본(Source)을 가리키는 심볼릭 링크를 생성합니다.
#   파일과 디렉터리 모두 지원합니다.
#   - 소스가 디렉터리(또는 아직 없음)이면 mkdir -p 로 생성합니다.
#   - 소스가 파일이면 mkdir 을 건너뜁니다 (파일을 디렉터리로 덮어쓰지 않음).
#   - 이미 대상 위치에 잘못된 타겟을 가리키는 링크가 있거나, 일반 파일/폴더가 있으면
#     안전을 위해 백업 후 올바른 심볼릭 링크로 교체(복구)합니다.
#   - 끊어진 링크(Broken Symlink) 방지를 보장하는 가장 안전한 심볼릭 링크 유틸리티입니다.
#
# 예시 (디렉터리):
#   ./create-symbolic-link.sh "/home/user/data/my_target" "/home/user/my_link"
# 예시 (파일):
#   ./create-symbolic-link.sh "/home/user/config/settings.json" "/home/user/.config/Code/User/settings.json"
# ==============================================================================

if [ "$#" -ne 2 ]; then
    echo "사용법: $0 <원본_경로> <생성할_링크_경로>"
    exit 1
fi

source_dir="$1"
target_link="$2"

# 1. 소스 보장 — 파일이면 mkdir 스킵, 디렉터리이거나 아직 없는 경우에만 생성
if [ ! -f "$source_dir" ]; then
    mkdir -p "$source_dir"
fi

# 2. 타겟 링크 부모 디렉터리 보장
mkdir -p "$(dirname "$target_link")"

if [ -L "$target_link" ]; then
    current_target=$(readlink -f "$target_link" 2>/dev/null || true)
    desired_target=$(readlink -f "$source_dir")

    if [ "$current_target" = "$desired_target" ]; then
        echo "[확인] $target_link 이미 올바른 심볼릭 링크입니다."
        exit 0 # 올바른 링크이므로 밑의 ln -s를 실행하지 않고 즉시 정상 종료
    else
        echo "[복구] $target_link 대상이 잘못되어 기존 링크 삭제 후 재연결합니다."
        rm -rf "$target_link"
    fi
elif [ -e "$target_link" ]; then
    bak="${target_link}.backup.$(date +%s)"
    mv "$target_link" "$bak"
    echo "[복구] $target_link 위치에 일반 디렉터리나 파일이 있어 백업($bak) 후 심볼릭 링크로 교체합니다."
fi

ln -s "$source_dir" "$target_link"
echo "[성공] $target_link -> $source_dir"
