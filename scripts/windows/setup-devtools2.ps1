...
# 그룹 권한이 현재 WSL 세션에 즉시 반영되지 않을 수 있으므로
# sg(switch group) 명령으로 devers 그룹 컨텍스트를 강제 적용하여 실행합니다.
Write-SubStep "▶ (1/3) WSL2 환경 변수 주입 (~/.bashrc)"
wsl -d $wslDistro -- bash -lc "sg devers -c 'bash /var/opt/_devtools2/scripts/linux/devtools2/1.setup-env.sh'"
...
wsl -d $wslDistro -- bash -lc "sg devers -c 'bash /var/opt/_devtools2/scripts/linux/devtools2/2.install-core-tools.sh'"
if ($LASTEXITCODE -ne 0) { Write-Fail "핵심 도구 설치 실패"; Pause-Script; exit 1 }

Write-SubStep "▶ (3/3) WSL2 CLI 유틸리티 및 apt 패키지 설치"
wsl -d $wslDistro -- bash -lc "sg devers -c 'bash /var/opt/_devtools2/scripts/linux/devtools2/3.install-cli-tools.sh'"
