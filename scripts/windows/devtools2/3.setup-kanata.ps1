...
Write-Host "⌨️  kanata 설치 및 CapsLock (ESC/Ctrl) Windows 전역 키 매핑" -ForegroundColor Magenta
Write-Host "===========================================================================" -ForegroundColor Magenta
Write-Host ""
...
if ($doDownload) {
    Write-Host "  📦 GitHub 최신 릴리스에서 kanata 바이너리 다운로드 중..." -ForegroundColor White
    
    # 프로세스 실행 중이면 임시 종료
    Get-Process kanata -ErrorAction SilentlyContinue | Stop-Process -Force -ErrorAction SilentlyContinue

    $downloadUrl = "https://github.com/jtroo/kanata/releases/latest/download/kanata.exe"
    $downloadUrlAlt = "https://github.com/jtroo/kanata/releases/latest/download/kanata_winstack.exe"

    try {
        Invoke-WebRequest -Uri $downloadUrl -OutFile $kanataExe -UseBasicParsing -ErrorAction Stop
        Write-Success "kanata.exe 다운로드 완료"
    } catch {
        try {
            Write-Host "  기본 URL 실패, 대체 URL(kanata_winstack.exe)로 재시도..." -ForegroundColor Yellow
            Invoke-WebRequest -Uri $downloadUrlAlt -OutFile $kanataExe -UseBasicParsing -ErrorAction Stop
            Write-Success "kanata_winstack.exe 다운로드 및 kanata.exe 저장 완료"
        } catch {
...
try {
    ...
    try {
        ...
        Register-ScheduledTask -TaskName $taskName -Action $action -Trigger $trigger -Principal $principal -Settings $settings -Force | Out-Null
        Write-Success "Windows 작업 스케줄러 등록 완료 ($taskName - 사용자 권한)"
    } catch {
