...
# 1) WezTerm 공식 실행 파일 4종만 명시적 화이트리스트로 지정하여 웹 차단 해제(Unblock-File)
$wezDirs = @("$env:ProgramFiles\WezTerm", "${env:ProgramFiles(x86)}\WezTerm")
$officialExes = @("wezterm-gui.exe", "wezterm.exe", "wezterm-mux-server.exe", "open-wezterm-here.exe")

foreach ($dir in $wezDirs) {
    if (Test-Path $dir) {
        foreach ($exeName in $officialExes) {
            $exePath = Join-Path $dir $exeName
            if (Test-Path $exePath) {
                Unblock-File -Path $exePath -ErrorAction SilentlyContinue
            }
        }
        Write-Success "WezTerm 공식 실행 파일 웹 차단 해제 완료: $dir"
    }
}
