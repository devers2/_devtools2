...
# 5. 동기식 직접 실행 원칙 (Start-Job / 백그라운드 스피너 금지):
#    - 과거에는 다운로드/압축 해제 시 회전 스피너를 돌리기 위해 Start-Job 백그라운드 프로세스를 사용했으나,
#      메인 스레드의 캐리지 리턴(\r) 스피너 루프와 서브 프로세스 간의 콘솔 버퍼 경합으로 인해 Windows 기본
#      콘솔(conhost.exe)의 뷰포트 좌표가 왜곡되어 이전 WSL 출력 줄이 겹쳐 깨지는 부작용이 발생했습니다.
#    - 또한 AHK v2 zip 패키지는 1~2MB 수준으로 직접 실행해도 1~2초 내에 완료되므로, Job 생성 오버헤드(3~6초)와
#      COM 객체 직렬화 오류 위험만 가중시킵니다.
#    - 따라서 모든 다운로드(Invoke-WebRequest -UseBasicParsing), 압축 해제(Expand-Archive), 작업 스케줄러 등록은
#      절대로 Start-Job을 쓰지 말고 메인 스레드에서 직접 동기(Synchronous) 실행해야 하며, 상태 알림은
#      Write-Info / Write-Success 기반의 순차 텍스트 로그로 처리해야 합니다.
# ------------------------------------------------------------------------------
# ==============================================================================
